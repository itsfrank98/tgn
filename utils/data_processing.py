...
def get_data_with_interaction(dataset_name, different_new_nodes_between_val_and_test=False, consider_synthetic=False):
    ...
    edge_idxs = graph_df.edge_idx.values
    node_idxs = graph_df.node_idx.values
    global_idx = graph_df.global_idx.values
    labels = graph_df.label.values
    timestamps = graph_df.ts.values
    types = graph_df.type.values
    full_data = Data(sources=sources, destinations=destinations, timestamps=timestamps, labels=labels, global_idxs=global_idx,
                     edge_idxs=edge_idxs, node_idxs=node_idxs, interaction_types=types)
    ...
    test_node_set = set(sources[(timestamps > val_time) & (timestamps < inf_time)]).union(set(destinations[(timestamps > val_time) & (timestamps < inf_time)]))
    ...
    train_data = Data(sources=sources[train_mask], destinations=destinations[train_mask], timestamps=timestamps[train_mask],
                      edge_idxs=edge_idxs[train_mask], labels=labels[train_mask], interaction_types=types[train_mask],
                      global_idxs=global_idx[train_mask], node_idxs=node_idxs[train_mask])
    ...
    val_data = Data(sources=sources[val_mask], destinations=destinations[val_mask], timestamps=timestamps[val_mask],
                    global_idxs=global_idx[val_mask], edge_idxs=edge_idxs[val_mask], node_idxs=node_idxs[val_mask],
                    labels=labels[val_mask], interaction_types=types[val_mask])

    test_data = Data(sources=sources[test_mask], destinations=destinations[test_mask], timestamps=timestamps[test_mask],
                    global_idxs=global_idx[test_mask], edge_idxs=edge_idxs[test_mask], node_idxs=node_idxs[test_mask],
                    labels=labels[test_mask], interaction_types=types[test_mask])

    # validation and test with edges that at least has one new node (not in training set)
    new_node_val_data = Data(sources=sources[new_node_val_mask], destinations=destinations[new_node_val_mask], timestamps=timestamps[new_node_val_mask],
                    global_idxs=global_idx[new_node_val_mask], edge_idxs=edge_idxs[new_node_val_mask], node_idxs=node_idxs[new_node_val_mask],
                    labels=labels[new_node_val_mask], interaction_types=types[new_node_val_mask])
